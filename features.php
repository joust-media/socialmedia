...
if (!$client) {
    ...
    echo '<p style="font:15px sans-serif;padding:40px;">
            ...
            <a href="admin">the admin picker</a>.
          </p>';
    exit;
}
...
function featureUrl($extra = []) {
    ...
    return 'features?' . http_build_query(array_filter($qs, fn($v) => $v !== null && $v !== ''));
}
?>
<!DOCTYPE html>
...
<script>
  ...
  document.addEventListener('click',async e=>{
    ...
    try{const fd=new FormData();fd.append('id',imageId);fd.append('status',newStatus);fd.append('actor','client');const r=await fetch('tire-status.php',{method:'POST',body:fd}),d=await r.json();if(!d.ok)throw new Error(d.error||'Failed');
  ...
  document.addEventListener('click',async e=>{
    ...
    try{const fd=new FormData();fd.append('id',imageId);fd.append('comment',text);fd.append('actor','client');const r=await fetch('tire-status.php',{method:'POST',body:fd}),d=await r.json();if(!d.ok)throw new Error(d.error||'Failed');
