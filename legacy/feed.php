<?php
/**
 * LEGACY — the pre-redesign Social Media Approval Feed, kept under legacy/
 * until posts.php reaches parity. The root feed.php now 301s to posts.php.
 ...
 * shell's static asset URLs and the active-tab detection still point at the
 * app root rather than at /legacy/.
 */
...
function buildFilterUrl($cats, $month, $statuses = null) {
    ...
    return legacyUrl('legacy/feed.php') . ($qs ? '?' . http_build_query($qs) : '');
}
?>
<!DOCTYPE html>
        ...
        <div class="post-media count-<?= $count ?> <?= $isCarousel ? 'is-carousel' : '' ?>"
          ...
          ?>
            <div class="media-item" data-image-id="<?= (int)$imgId ?>" data-media-type="<?= h($mtype) ?>">
              ...
              <?php else: ?>
                <img src="<?= h(legacyUrl($src)) ?>" alt="Post image <?= $i + 1 ?>" loading="lazy" data-image-el>
              <?php endif; ?>
              <button class="save-img-btn"
                      data-src="<?= h(legacyUrl($src)) ?>"
...
<script>
  ...
  const APP_BASE    = <?= json_encode(basePath(), JSON_UNESCAPED_SLASHES) ?>;
  const LEGACY_SELF = <?= json_encode(legacyUrl('legacy/feed.php'), JSON_UNESCAPED_SLASHES) ?>;
  ...
  document.getElementById('feed').addEventListener('click', async (e) => {
    ...
    if (decideBtn) {
      ...
      try {
        ...
        const res = await fetch(APP_BASE + '/status.php', { method: 'POST', body: formData });
    ...
    if (typeBadge) {
      ...
      picker.addEventListener('click', async (pev) => {
        ...
        try {
          ...
          const res = await fetch(APP_BASE + '/status.php', { method: 'POST', body: fd });
          const data = await res.json();
          if (!data.ok) throw new Error(data.error || 'Failed');
  ...
  replaceInput.addEventListener('change', async () => {
    ...
    try {
      ...
      const res = await fetch(APP_BASE + '/replace-image.php', { method: 'POST', body: fd });
      const data = await res.json();
      if (!data.ok) throw new Error(data.error || 'Failed');
      ...
      const newUrl = APP_BASE + '/' + data.image_url.replace(/^\/+/, '');
      ...
      if (saveBtn) {
        saveBtn.setAttribute('data-src', newUrl);
  ...
  if (monthSelect) {
    monthSelect.addEventListener('change', () => {
      ...
      window.location.href = LEGACY_SELF + (qs ? '?' + qs : '');
    });
  }
